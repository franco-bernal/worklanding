...
@section('content')
            ...
            <div class="fbg_table">
                ...
                @forelse($emails as $mail)
                    <div class="fbg_table_item" id="item-mensaje{{ $mail->id }}">
                        ...
                        <a style="float:right" class="btn btn-solid"
                            onclick="vermensaje('#vermensaje{{ $mail->id }}')">Mensaje</a>
                        <div class="fbg_vermensaje" id="vermensaje{{ $mail->id }}">
                            <span class="fbg_delete" onclick="deleteMensaje('{{ $mail->id }}')">borrar</span>
                            <a href="mailto:{{ $mail->email }}" class="m-email"><strong>Email:</strong>
                                {{ $mail->email }}</a>
                            <br>
                            <a href="tel:{{ $mail->numero }}" class="m-numero"><strong>Numero:</strong>
                                {{ $mail->numero }}</a>
        ...
        <div class="column is-4">
            ...
            <div class="visitas">
                @forelse($visitas as $visita)
                    <p id="visita{{ $visita->id }}" onclick="deleteVisita('{{ $visita->id }}',false)">
                        {{ todayDate($visita->created_at) }} - <strong>{{ $visita->ip }}</strong></p>
                @empty
                    <p>Sin visitas</p>
                @endforelse
            </div>
    ...
    <div class="columns">
        <div class="column is-12">
            <h3>Custom Css</h3>
            <p>CSS personalizado para la landing</p>
            <br>
            <form method='post' action="{{ route('customcss.edit') }}" class="form space-x">
                @csrf
                <textarea placeholder="Puedes escribir tu css aquí..." name="css"
                    style="width:100%; min-height:500px; display:block; margin: 0 auto; border-radius:5px; border: 1px solid: gray; resize: none; color:black; padding:1%;">{{ $customCss[0]->html_content }}</textarea>
