...
@section('content')
        ...
        @forelse(json_decode($proyect->steps) as $step)
            <div class="step" id="step-{{ $step }}">
                ...
                <div>
                                ...
                                <a class="task" id="taskElement{{ $task->id }}">
                                    <div class="selectStepInput">
                                        @foreach (json_decode($proyect->steps) as $stepe)
                                            <span
                                                onclick="moveTo('{{ $task->id }}','{{ $stepe }}','#taskElement{{ $task->id }}')">{{ $stepe }}</span>
                                        @endforeach
                                    </div>
                    ...
                    <div class="taskADD">
                        <a onclick="addStep('{{ $step }}')"><img id="btn-img" class="centrar"
                                src="{{ asset('img/circulo-plus.png') }}" alt=""></a>
                        ...
                        <form id="form-{{ $step }}" class="stepForm">
                            <br>
                            <textarea placeholder="Titulo de la tarea" id="txt{{ $step }}-name"
                                rows="4"></textarea>
                            <input type="hidden" value="{{ $step }}">
                            <a class="btn-square"
                                onclick="addTaskForm('{{ $proyect->id }}','{{ $step }}','txt{{ $step }}-name')">Crear</a>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <div class="taskView">
        <div class="taskSpace">
            ...
            <div class='columns is-mobile is-gapless is-multiline proyects'>
                <div class='column is-12-fullhd is-12-desktop  is-12-tablet  is-12-mobile stepModal'
                    style='margin: 10px 0;'>
                ...
                <div class='column is-8-fullhd is-8-desktop  is-12-tablet  is-12-mobile'>
                    ...
                    <div class="subtareas listTasked">
                        <div>
                            ...
                            <button class="btn-solid" id="btn-addSubTask"><img
                                    src="{{ asset('img/circulo-plus.png') }}" alt=""></button>
                        </div>
                        <div id="htmlSubTasks">
                            <!-- Creation Task -->
                        </div>
                    ...
                    <div class="checkList listTasked">
                        <div>
                            ...
                            <button class="btn-solid" id="btn-addCheckList"><img
                                    src="{{ asset('img/circulo-plus.png') }}" alt=""></button>
                        </div>
                        <div id="htmlLists">
                            <!-- Create List -->
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data != null) {
                        ...
                        steps.map(function(step) {
                            $(".stepModal").append(
                                `<span id="target${step}" class='stepTask'  onclick="moveTo('${data.id}','${step}','#taskElement${data.id}')">${step}</span>`
                            );
                        })
                        ...
                        $("#btn-addSubTask").attr('onclick',
                            `addJsonHtml('input-subTask', 'subTasks','${data.id}')`);
                        $("#btn-addCheckList").attr('onclick',
                            `addJsonHtml('input-checkList', 'checkLists','${data.id}')`);
                        $("#btn-addComment").attr('onclick',
                            `addJsonHtml('input-comment', 'comments','${data.id}')`);
                        ...
                        JSON.parse(data.checkLists).map((list) => {
                            $("#htmlLists").append(
                                `<span id='${list.id}checkLists'><a onclick="deleteJsonHtml('checkLists','${data.id}',${list.id},'${list.created_at}')">&#10060;</a><p onclick="checkSub('checkLists','${data.id}','${list.id}')"  style="${list.check == 1 ? 'text-decoration: line-through !important;':''}">${list.detail}</p></span>`
                            );
                        });
