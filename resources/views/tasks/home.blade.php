...
@section('content')
    ...
    <div id="columnFavorite" class='columns is-mobile is-gapless is-multiline proyects'>
        ...
        @forelse ($proyects as $proyect)
            @if ($proyect->favorite == true)
                <div id="proyect{{ $proyect->id }}" class=' column is-4-fullhd is-4-desktop  is-6-tablet  is-6-mobile'
                    style="padding:10px !important;">
                    <div class="proyect" style="background-image:url('{{ $proyect->img_link }}')">
                        <a href="{{ route('task.proyect', ['id' => $proyect->id]) }}"
                            class="py-4">{{ $proyect->title }}</a>
                ...
                <div id="proyect{{ $proyect->id }}" class=' column is-4-fullhd is-4-desktop  is-6-tablet  is-6-mobile'
                    ...
                    <div class="proyect" style="background-image:url('{{ $proyect->img_link }}')">
                        <a href="{{ route('task.proyect', ['id' => $proyect->id]) }}"
                            class="py-4">{{ $proyect->title }}</a>
                        <span class="fecha_proyect">{{ $proyect->created_at }}</span>
                        <span class="star" data-favorite="false"
                            onclick="favorite('{{ $proyect->id }}')">★</span>
                    </div>
                </div>
            @else
            @endif
    ...
    <div class="taskSpace">
        ...
        <form method='post' action="{{ route('task.add') }}" class="form space-x" id="updateBlog"
